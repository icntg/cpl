#ifndef CRYPTO_HTTP_H_SUPPORTER_VEHICLE_STRONG_GLIDER_MOTION_PATIENT_JOURNEY
#define CRYPTO_HTTP_H_SUPPORTER_VEHICLE_STRONG_GLIDER_MOTION_PATIENT_JOURNEY
...
namespace crypto {
    ...
    public:
        ...
        int32_t Encrypt(
            ...
            if (0 != retCode) {
                log_error("[x] Crypto$$$Encrypt failed %ld", retCode);
                goto __ERROR__;
            }
            pv_hash = &hash;
    ...
    static int32_t Decrypt(_In_ const string &key, _In_ const string &encrypted, _Out_ string &plain) {
        ...
        if (0 != retCode) {
            log_error("[x] Crypto$$$DecryptStream failed %ld", retCode);
            goto __ERROR__;
        }
...
#endif //CRYPTO_HTTP_H_SUPPORTER_VEHICLE_STRONG_GLIDER_MOTION_PATIENT_JOURNEY
