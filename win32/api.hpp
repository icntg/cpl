    ...
    public:
        ...
        Int32Result Load() override {
            for (const auto& dll : DLL_NAMES) {
                ...
                const auto ret_InternetSetOptionA = api::DynamicModule::LoadFunction<::cpl::sys::api::WinINet::InternetSetOptionA>("InternetSetOptionA");
                if (!ret_InternetSetOptionA) {
                    ok = false;
                } else {
                    any_loaded = true;
                    InternetSetOptionA = ret_InternetSetOptionA.value();
                }
                if (ok && (any_loaded || DLL_NAMES.size() == 1)) {
                    return 0;
                }
