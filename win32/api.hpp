    ...
    public:
        ...
        Int32Result Load() override {
            for (const auto& dll : DLL_NAMES) {
                ...
                const auto ret_getaddrinfo = api::DynamicModule::LoadFunction<::cpl::sys::api::Ws2_32::getaddrinfo>("getaddrinfo", false);
                if (!ret_getaddrinfo) {
                } else {
                    any_loaded = true;
                    getaddrinfo = ret_getaddrinfo.value<>();
                }
                const auto ret_freeaddrinfo = api::DynamicModule::LoadFunction<::cpl::sys::api::Ws2_32::freeaddrinfo>("freeaddrinfo", false);
                if (!ret_freeaddrinfo) {
                } else {
                    any_loaded = true;
                    freeaddrinfo = ret_freeaddrinfo.value<>();
                }
                if (ok && (any_loaded || DLL_NAMES.size() == 1)) {
                    return 0;
                }
    ...
    public:
        ...
        Int32Result Load() override {
            for (const auto& dll : DLL_NAMES) {
                ...
                const auto ret_NotifyAddrChange = api::DynamicModule::LoadFunction<::cpl::sys::api::IPv4::NotifyAddrChange>("NotifyAddrChange", false);
                if (!ret_NotifyAddrChange) {
                } else {
                    any_loaded = true;
                    NotifyAddrChange = ret_NotifyAddrChange.value<>();
                }
                const auto ret_NotifyRouteChange = api::DynamicModule::LoadFunction<::cpl::sys::api::IPv4::NotifyRouteChange>("NotifyRouteChange", false);
                if (!ret_NotifyRouteChange) {
                } else {
                    any_loaded = true;
                    NotifyRouteChange = ret_NotifyRouteChange.value<>();
                }
                if (ok && (any_loaded || DLL_NAMES.size() == 1)) {
                    return 0;
                }
        ...
        Int32Result Unload() override {
            ...
            NotifyAddrChange = nullptr;
            NotifyRouteChange = nullptr;
            const auto unloadRet = api::DynamicModule::Unload();
            if (!unloadRet) {
                return unloadRet;
            }
    ...
    public:
        ...
        Int32Result Load() override {
            for (const auto& dll : DLL_NAMES) {
                ...
                const auto ret_RegisterDeviceNotificationW = api::DynamicModule::LoadFunction<::cpl::sys::api::User32::RegisterDeviceNotificationW>("RegisterDeviceNotificationW", false);
                if (!ret_RegisterDeviceNotificationW) {
                } else {
                    any_loaded = true;
                    RegisterDeviceNotificationW = ret_RegisterDeviceNotificationW.value<>();
                }
                const auto ret_UnregisterDeviceNotification = api::DynamicModule::LoadFunction<::cpl::sys::api::User32::UnregisterDeviceNotification>("UnregisterDeviceNotification", false);
                if (!ret_UnregisterDeviceNotification) {
                } else {
                    any_loaded = true;
                    UnregisterDeviceNotification = ret_UnregisterDeviceNotification.value<>();
                }
                if (ok && (any_loaded || DLL_NAMES.size() == 1)) {
                    return 0;
                }
        ...
        Int32Result Unload() override {
            ...
            RegisterDeviceNotificationW = nullptr;
            UnregisterDeviceNotification = nullptr;
            const auto unloadRet = api::DynamicModule::Unload();
            if (!unloadRet) {
                return unloadRet;
            }
